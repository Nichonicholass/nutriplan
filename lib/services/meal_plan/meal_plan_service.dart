...
class MealPlanService {
  ...
  MealItem _pickMeal({
    ...
    if (candidates.isEmpty) return _fallbackMeal(mealType);

    // Score each candidate and pick from top 3 (smart + some variety)
    candidates.sort((a, b) {
      final sa = _scoreFood(a, goalKey, budgetPerMeal, preferences);
      final sb = _scoreFood(b, goalKey, budgetPerMeal, preferences);
      return sb.compareTo(sa);
    });

    final top = candidates.take(3).toList();
    return top[_rng.nextInt(top.length)].toMealItem(mealType);
  }
  ...
  double _scoreFood(
    ...
    // 1. Nutritional fit (0–40 pts)
    switch (goalKey) {
      case 'bulking':
        score += (food.protein * 1.2).clamp(0, 40);
        break;
      case 'cutting':
        score += ((40 - (food.calories - 250) / 12)).clamp(0, 40);
        break;
      case 'diabetes':
        score += (40 - food.carbs * 0.4).clamp(0, 40);
        break;
      case 'stunting':
        score += (food.calories / 18 + food.protein * 0.5).clamp(0, 40);
        break;
      default: // healthy
        score += (30 - (food.protein - 20).abs() * 0.5).clamp(0, 40);
    }

    // 2. Budget efficiency (0–30 pts)
    if (budgetPerMeal > 0) {
      final ratio = food.estimatedCost / budgetPerMeal;
      final budgetScore = ratio <= 1.0 ? 30 * ratio : 30 * (2.0 - ratio);
      score += budgetScore.clamp(0, 30);
    }

    // 3. Preference keyword match (0–20 pts)
    if (preferences.isNotEmpty) {
      final tokens = preferences
          .toLowerCase()
          .split(RegExp(r'[\s,]+'))
          .where((t) => t.length > 2)
          .toList();
      final searchTarget =
          '${food.name} ${food.ingredients.join(' ')}'.toLowerCase();
      if (tokens.any((t) => searchTarget.contains(t))) {
        score += 20;
      }
    }

    // 4. Goal tag priority (0–10 pts)
    if (food.goals.contains(goalKey)) {
      score += 10;
    } else if (food.goals.contains('healthy')) {
      score += 5;
    }
...
class MealPlanException implements Exception {
  ...
  @override
  String toString() => 'MealPlanException: $message';
}
